...
class HELPER:
    ...
    def gettestData(self, tickers, indx, attrs, test_dir):
        """
        Return DataFrame with data for a list of tickers plus and index
        ...
        - Directory of test data. If not provided, assumes training data.
        ...
        test_dfs = []
        for ticker_num, ticker in enumerate(tickers):
            ticker_file = os.path.join(test_dir, "{t}.csv".format(t=ticker))
            ticker_df = pd.read_csv(ticker_file, index_col=dateAttr, usecols=use_cols)

            # Rename attributes with ticker name for the test data
            ticker_df = self.attrRename(ticker_df, f"{ticker}")

            test_dfs.append(ticker_df)

        # Concatenate the test data DataFrames
        test_data_df = pd.concat(test_dfs, axis=1)

        # Concatenate the training and test data DataFrames
        data_df = pd.concat(dfs, axis=1)
        data_df = pd.concat([data_df, test_data_df], axis=0)
